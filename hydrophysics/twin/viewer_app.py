...
function recolour() {
  ...
  for (const rec of meshes) {
    ...
    for (let i = 0; i < A; i++) {
      let c;
      if (kind === "ground") {
        c = ramp(subsRamp, subsAt(i, t) / subsMax);
...
function pick(e) {
  ...
  if (hits.length) { state.sel = hits[0].instanceId; drawProbe(); }
}
...
$("startYear").max = 1;
place(); applyVisibility(); setMode("decide"); refresh();
